...
export function ProjectCard({ project }: { project: Project }) {
  ...
  const shadowColor = project.category === 'systems' ? '#ff2a85' : project.category === 'mobile' ? '#4ade80' : '#00f0ff'
    ...
    <motion.article initial={false} whileHover={reducedMotion ? undefined : { x: -2, y: -2, boxShadow: `6px 6px 0 ${shadowColor}` }} transition={{ duration: reducedMotion ? 0 : 0.18 }} className={`flex min-w-0 flex-col border-2 border-border bg-card ${color.shadow}`}>
      <figure className="border-b-2 border-border">
        <div className="relative">
          ...
          <span className={`absolute left-3 top-3 border border-border px-2 py-1 font-mono text-[10px] font-bold text-black ${color.badge}`}>{project.categoryLabel}</span>
        </div>
        <figcaption className="px-3 py-2 text-xs text-muted-foreground">Ilustrasi sementara; bukan tampilan proyek.</figcaption>
      </figure>
      <div className="flex flex-1 flex-col gap-3 p-5">
        <div className="flex flex-wrap justify-between gap-2 font-mono text-xs text-muted-foreground">
          <span>ORGANISASI: {project.client}</span><span>{project.period}</span>
        </div>
        ...
        {project.tags.length > 0 && <ul className="flex flex-wrap gap-1.5 pt-2" aria-label="Teknologi proyek">{project.tags.map((tag) => <li className="tag" key={tag}>{tag}</li>)}</ul>}
