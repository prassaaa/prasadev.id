      ...
      <div className="site-container">
        ...
        <div className="grid gap-6 md:grid-cols-3">{principles.map((principle) => {
          ...
          return <article key={principle.number} className={`flex min-w-0 flex-col justify-between border-2 border-border bg-card p-6 ${shadows[principle.accent]}`}>
            <div className="space-y-4"><div className={`flex size-12 items-center justify-center border-2 border-border text-xl font-black text-black shadow-neo ${accents[principle.accent]}`}>{principle.number}</div><h3 className="font-display text-2xl font-bold">{principle.title}</h3><p className="text-sm leading-relaxed text-muted-foreground sm:text-base">{principle.description}</p></div>
            <p className="mt-6 flex items-center gap-2 border-t-2 border-border/20 pt-4 font-mono text-xs font-bold"><Icon className="size-4 shrink-0" aria-hidden="true" />{principle.detail}</p>
          </article>
        })}</div>
      </div>
    </section>
