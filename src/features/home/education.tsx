    ...
    <section id="education" className="section-shell bg-muted">
      <div className="site-container">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {education.contributions.map((item, index) => (
            <article
              key={item.title}
              className={`min-w-0 border-2 border-border bg-card p-6 ${shadows[index]}`}>
        ...
        <div className="mt-10 border-2 border-border bg-card p-6 sm:p-8">
          <h3 className="mb-6 font-display text-2xl font-bold">Pengalaman Organisasi</h3>
          <ul className="space-y-5">
            {education.organizations.map((item) => (
              <li key={item.name} className="border-l-2 border-neo-pink pl-4">
                <p className="font-mono text-xs font-bold text-muted-foreground">{item.period}</p>
