        ...
        <div className="min-w-0 lg:col-span-7">
          <div className="border-2 border-border bg-card p-6 shadow-neo-cyan sm:p-10">
            ...
            <div className="mt-8 flex flex-col items-start gap-5">
              <Button asChild size="lg" className="w-full whitespace-normal bg-neo-lime shadow-neo sm:w-auto"><a href={site.whatsapp.href} target="_blank" rel="noopener noreferrer"><PhoneCall aria-hidden="true" />Diskusi via WhatsApp<ArrowUpRight aria-hidden="true" /></a></Button>
              <Button asChild size="lg" variant="outline" className="w-full whitespace-normal sm:w-auto"><a href={site.email.href}><Mail aria-hidden="true" />Kirim Email</a></Button>
              {site.socials.map((social) => <Button key={social.name} asChild variant="outline" className="w-full whitespace-normal sm:w-auto"><a href={social.href} target="_blank" rel="noopener noreferrer" aria-label={social.label}>Terhubung di {social.name}<ArrowUpRight aria-hidden="true" /></a></Button>)}
            </div>
          </div>
        </div>
      </div>
