...
const colors = { yellow: 'bg-neo-yellow', pink: 'bg-neo-pink', cyan: 'bg-neo-cyan', lime: 'bg-neo-lime' }
const shadows = { yellow: 'shadow-neo-cyan', pink: 'shadow-neo-pink', cyan: 'shadow-neo-cyan', lime: 'shadow-neo-lime' }

export function Services() {
  return (
    <section id="services" className="section-shell bg-background">
      <div className="site-container">
        <div className="mb-12 flex flex-col justify-between gap-6 md:flex-row md:items-end"><SectionHeading label={site.services.eyebrow} accent="cyan" className="mb-0 max-w-2xl">{site.services.title}</SectionHeading><Button asChild variant="outline" className="max-w-full self-start whitespace-normal md:self-auto"><Link to="/#contact">{site.services.cta}<ArrowRight aria-hidden="true" /></Link></Button></div>
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">{services.map((service) => {
          const Icon = icons[service.icon]
          return <article key={service.title} className={`flex min-w-0 flex-col border-2 border-border bg-card p-6 ${shadows[service.accent]}`}>
