...
export function SiteHeader({ theme, onToggleTheme }: { theme: 'dark' | 'light'; onToggleTheme: () => void }) {
  ...
  return <>
    <a href="#main-content" className="fixed top-2 left-2 z-[100] -translate-y-24 bg-primary px-4 py-3 font-bold text-black focus:translate-y-0">Lewati ke konten utama</a>
    ...
    <header className="sticky top-0 z-50 border-b-2 bg-background" onKeyDown={(event) => { if (event.key === 'Escape' && open) { setOpen(false); menuButton.current?.focus() } }}>
      ...
      <AnimatePresence initial={false}>{open && <motion.nav key="mobile" id="mobile-navigation" initial={reducedMotion ? false : { height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: reducedMotion ? 0 : 0.18 }} aria-label="Navigasi mobile" className="overflow-hidden bg-muted xl:hidden"><div className="site-container border-t-2 py-4">{navigation.map(([label, href]) => <Link key={href} to={href} onClick={() => setOpen(false)} className="block px-3 py-2 font-bold hover:bg-primary hover:text-black">{label}</Link>)}<Button asChild className="mt-3 w-full"><Link to="/#contact" onClick={() => setOpen(false)}>Mulai Diskusi Proyek</Link></Button></div></motion.nav>}</AnimatePresence>
    </header>
  </>
}
